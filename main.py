...
CONSUMER_KEY = 'XgXHacoQ2iQVxbs9Ja1IHw'
CONSUMER_SECRET = 'Z31Y9puT7ZTRnXDVU3UgLcWzjzYnk18BLhQakJt8'
        ...
        if 'Authorization' in headers :
            ...
            if 'oauth_version' in auth_header :
                ...
                if username is None :
                    ...
                    protected=True
                    user_access_token, user_access_secret, user_id = client.get_access_from_db(username, password)
...
class OauthPage(webapp.RequestHandler):
    ...
    def get(self, mode=""):
        ...
        if mode=='verify':
            ...
            try:
                access_token, access_secret, screen_name, user_id = client.get_access_token(auth_token, auth_verifier)
                self_key = '%s' % uuid4()
                # Save the auth token and secret in our database.
                client.save_user_info_into_db(username=screen_name, userid=user_id, password=self_key, 
                                              token=access_token, secret=access_secret)
                show_key_url = '%s/oauth/showkey?name=%s&userid=%s&key=%s' % (
                                                                       self.request.host_url, 
                                                                       screen_name, user_id, self_key)
        ...
        if mode=='showkey':
            ...
            out_message = """
                ...
                the Key of this API : <b>#self_key#</b> <a href="#api_host#/oauth/change?name=#screen_name#&userid=#user_id#&key=#self_key#">you can change it now</a><br /><br />
                </p>
                <p>
            ...
            out_message = out_message.replace('#user_id#', user_id)
        ...
        if mode=='change':
            ...
            out_message = """
                ...
                your user id of Twitter : <input type="text" name="userid" size="20" value="%s"> <br /><br />
                ...
                """ % (self.request.host_url, screen_name, user_id, self_key)
            self.response.out.write( out_message )
    ...
    def post(self, mode=''):
        ...
        if mode=='access_token':
            ...
            oauth_token, oauth_token_secret, user_id = client.get_access_from_db(screen_name, self_key)
            self.response.out.write( 'oauth_token=%s&oauth_token_secret=%s&user_id=%s&screen_name=%s&x_auth_expires=0' % (oauth_token, oauth_token_secret, user_id, screen_name) )

        if mode=='changekey':
            screen_name = self.request.get("name")
            old_key = self.request.get("old_key")
            new_key = self.request.get("new_key")
            user_access_token, user_access_secret, user_id = client.get_access_from_db(screen_name, old_key)
            ...
            if user_access_token is None or user_access_secret is None:
                ...
                try:
                    client.save_user_info_into_db(username=screen_name, userid=user_id, password=new_key, 
                                                  token=user_access_token, secret=user_access_secret)
                    show_key_url = '%s/oauth/showkey?name=%s&userid=%s&key=%s' % (
                                                                        self.request.host_url, 
                                                                        screen_name, user_id, new_key)
                    self.redirect(show_key_url)
                except Exception,error_message:
                    logging.debug("screen_name:" + screen_name)
                    logging.debug("user_id:" + user_id)
