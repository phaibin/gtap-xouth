...
class OAuthClient():
    ...
    def get_access_token(self, auth_token, auth_verifier):
        ...
        return result['token'], result['secret'],result['screen_name'],result['user_id']
    ...
    def get_access_from_db(self, username, password):
        ...
        if not result:
            ...
            if result.token[:3]=='###' and result.secret[:3]=='###':
                ...
                access_userid = result.userid
            else:
                access_token = None
                access_secret = None
                access_userid = None
        return access_token, access_secret, access_userid
    ...
    def save_user_info_into_db(self, username, userid, password, token, secret):
